...
class PoseClassifier(object):
    ...
    def find_knees_tilt(self, pose_distance_error, T=30, alpha1=0.02, alpha2=0.05, alpha3=0.1, alpha4=0.25,
                        distance1=5, distance2=8, distance3=12, distance4=-2.5):
        '''我們會把用來預測的影片,其每張frame與正確姿勢計算得到的相對誤差,拿來做計算。
...
def video_prediction_output_video(video_path='IMG_0914.MOV', out_video_path='IMG_0914-output_t3.MOV'):
    ...
    with tqdm.tqdm(total=video_n_frames, position=0, leave=True) as pbar:
        ...
        switch = False
        knees_parallel_min_score = 0
        knees_tilt_min_score = 0
        # =========================
        while True:
            ...
            if pose_landmarks is not None:
                ...
                if switch == True:
                    switch_knees_parallel_list.append(knees_score_filtered)
                    switch_knees_tilt_list.append(knees_tilt_filtered)
            ...
            output_frame = pose_classification_visualizer(
                ...
                knees_tilt=math.floor(knees_tilt_min_score))
            # ----------------------------
            ...
            if frame_idx % 50 == 0:
                ...
                if frame_idx % 100 == 0:
                    ...
                    print(f'knees_parallel_min_score:{knees_parallel_min_score}')
                    print(f'knees_tilt_min_score: {knees_tilt_min_score}')
                # -----------------------
            frame_idx += 1
            pbar.update()
...
# selfchange_ed3_即時攝影鏡頭預測:
def camera_realtime_predict():
    class_name = 'down'
    ...
    # --------------------------- #selfchange_ed2&ed3_膝蓋平行&膝蓋傾斜分數平滑
    # Initialize knees score smoothing.
    ...
    while True:
                ...
                if switch_list[-2] == True:
                    if switch == False:
                        ...
                        final_list.append([repetition_counter.n_repeats,
                                           ...
                                           math.floor(knees_tilt_min_score)])
                        print(final_list)
                        # ==================
            # -------------------------------
        ...
        cv2.putText(output_frame, "Knees Tilt Score:", (int(video_width * 0.45), int(video_height * 0.5)),
                    cv2.FONT_HERSHEY_TRIPLEX, 1, (255, 0, 0), 2)
        cv2.putText(output_frame, str(math.floor(knees_tilt_min_score)),
                    (int(video_width * 0.7), int(video_height * 0.6)), cv2.FONT_HERSHEY_TRIPLEX, 2, (255, 0, 0), 2)
    ...
    return print(f'總共完成深蹲次數: {repetition_counter.n_repeats}')
