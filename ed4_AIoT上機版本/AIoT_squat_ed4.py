...
class FullBodyPoseEmbedder(object):
    ...
    def _get_pose_distance_embedding(self, landmarks):
        ...
        embedding = np.array([
            ...
            #----------------------- #selfchange_ed4_將肩膀連線的向量除以3，以降低肩膀向量在平均時的權重影響過大
            self._get_distance_by_names(landmarks, 'left_shoulder','left_knee')/5,
            self._get_distance_by_names(landmarks, 'right_shoulder','right_knee')/5,

            # Five joints.

            self._get_distance_by_names(landmarks, 'left_shoulder', 'left_ankle')/5,
            self._get_distance_by_names(landmarks, 'right_shoulder', 'right_ankle')/5,
            #-----------------------
...
class PoseClassifier(object):
    ...
    def compare_correct_pose_distance(self, pose_landmarks, class_name):
        ...
        # 計算距離特徵平均差距，從小到大排列並挑選出差距前10%小的圖片(有做鏡像處理) (2021/12/20)
        mean_dist_heap = []
        ...
        mean_dist_heap_idx = mean_dist_heap_idx[:math.floor(0.1 * self._num)]

        # 將上面取出前10%角度最接近的正確姿勢圖拿來與frame做距離特徵相減，並且取平均:
    ...
    def find_knees_score(self, pose_distance_error, T=30, alpha1=0.02, alpha2=0.05, alpha3=0.1, alpha4=0.1, distance1=5,
        ...
        方法: 會使用到 左膝蓋到右膝蓋的向量以及左腳踝到右腳踝的向量，將這兩個向量與正確姿勢平均向量相減，
            會得到兩個相對向量，再將膝蓋到膝蓋跟腳踝到腳踝兩個相對向量相減，可以得到一個比較向量。
            我們會透過比較向量的y軸向量絕對值來當作誤差距離
        '''
    ...
    # ------------------------------- #selfchange_ed3_自定義函數:計算膝蓋前傾評分 (2021/12/23)
    def find_knees_tilt(self, pose_distance_error, T=30, alpha1=0.02, alpha2=0.05, alpha3=0.1, alpha4=0.1, alpha5=0.3,
                        distance1=5, distance2=8, distance3=12, distance4=-3, distance5=-8):
        '''我們會把用來預測的影片,其每張frame與正確姿勢計算得到的相對誤差,拿來做計算。

        Required input: (13,3) distance error feature [from previous function]
        Output: one value which means knees_parallel_score
        方法: 會使用到 左膝蓋到左腳踝的向量以及右膝蓋到右腳踝的向量，將這兩個向量與正確姿勢平均向量相減，
            會得到兩個相對向量，再將兩段向量相加除以二找出膝蓋對腳踝平均的相對位置，可以得到一個比較向量。
            我們會透過比較向量的y軸向量來當作誤差距離
        '''

        distance = (pose_distance_error[3][1] + pose_distance_error[4][1]) / 2
        ...
        if distance < distance4:
            y6 = y5 * math.exp(alpha4 * (distance - distance4))
        else:
            y6 = y5
        if distance < distance5:
            y7 = y6 * math.exp(alpha5 * (distance - distance5))
        else:
            y7 = y6
    ...
    # ------------------------------- #selfchange_ed4_自定義函數:計算臀部下壓評分 (2021/12/28)
    def find_pipe_down(self, pose_distance_error, T=30, alpha1=0.02, alpha2=0.05, alpha3=0.1, alpha4=0.1, alpha5=0.3,
                       distance1=5, distance2=8, distance3=12, distance4=-3, distance5=-8):
        '''我們會把用來預測的影片,其每張frame與正確姿勢計算得到的相對誤差,拿來做計算。

        Required input: (13,3) distance error feature [from previous function]
        Output: one value which means knees_parallel_score
        方法: 會使用到 左臀部到左膝蓋的向量以及右臀部到右膝蓋的向量，將這兩個向量與正確姿勢平均向量相減，
            會得到兩個相對向量，再將左右臀部到膝蓋的兩個相對向量相加除以二，可以得到一個中心點的比較向量。
            我們會透過比較向量的y軸當作誤差距離。
        '''
        distance = (pose_distance_error[1][1] + pose_distance_error[2][1]) / 2
    ...
    # ------------------------------- #selfchange_ed4_計算背部挺直評分
    def find_back_straight(self, pose_distance_error, T=30, alpha1=0.02, alpha2=0.05, alpha3=0.1, alpha4=0.1,
                           alpha5=0.3,
                           distance1=5, distance2=8, distance3=12, distance4=-3, distance5=-8):
        '''我們會把用來預測的影片,其每張frame與正確姿勢計算得到的相對誤差,拿來做計算。
        ...
        Required input: (13,3) distance error feature [from previous function]
        Output: one value which means knees_parallel_score
        方法: 會使用到背部的向量，我們會透過比較向量的y軸當作誤差距離。
        '''
        distance = pose_distance_error[0][1]
...
class PoseClassificationVisualizer(object):
    ...
    def __call__(self,
        ...
        # ------------------------------- #selfchange_ed2&ed3&ed4_增加文字出現位置: (2021/12/22)
        text_location_x = 0.65
        text_location_x2 = 0.75
        text_location_y1 = 0.05
        text_location_y2 = 0.2
        text_location_y3 = 0.22
        text_location_y4 = 0.3
        text_location_y5 = 0.32
        text_location_y6 = 0.4
        text_location_y7 = 0.42
        text_location_y8 = 0.5
        text_location_y9 = 0.52
        ...
        counter_font = ImageFont.truetype("msjhbd.ttc", int(0.35 * output_height * self._counter_font_size),
                                          encoding='utf-8')
        output_img_draw.text((output_width * text_location_x,
                              output_height * text_location_y4),
                             "膝蓋傾斜評分:",
                             font=counter_font,  # *****
                             fill=self._counter_font_color)
        # (5)
        counter_font = ImageFont.truetype("msjhbd.ttc", int(0.6 * output_height * self._counter_font_size),
                                          encoding='utf-8')
        output_img_draw.text((output_width * text_location_x2,
                              output_height * text_location_y5),
                             str(knees_tilt),
                             font=counter_font,  # *****
                             fill=self._counter_font_color)
        ...
        counter_font = ImageFont.truetype("msjhbd.ttc", int(0.35 * output_height * self._counter_font_size),
                                          encoding='utf-8')
        output_img_draw.text((output_width * text_location_x,
                              output_height * text_location_y6),
                             "臀部下壓評分:",
                             font=counter_font,  # *****
                             fill=self._counter_font_color)
        # (7)
        counter_font = ImageFont.truetype("msjhbd.ttc", int(0.6 * output_height * self._counter_font_size),
                                          encoding='utf-8')
        output_img_draw.text((output_width * text_location_x2,
                              output_height * text_location_y7),
                             str(pipe_down),
                             font=counter_font,  # *****
                             fill=self._counter_font_color)
        # (8)
        counter_font = ImageFont.truetype("msjhbd.ttc", int(0.35 * output_height * self._counter_font_size),
                                          encoding='utf-8')
        output_img_draw.text((output_width * text_location_x,
                              output_height * text_location_y8),
                             "背部挺直評分:",
                             font=counter_font,  # *****
                             fill=self._counter_font_color)
        # (9)
        counter_font = ImageFont.truetype("msjhbd.ttc", int(0.6 * output_height * self._counter_font_size),
                                          encoding='utf-8')
        output_img_draw.text((output_width * text_location_x2,
                              output_height * text_location_y9),
                             str(back_straight),
                             font=counter_font,  # *****
                             fill=self._counter_font_color)
...
def video_prediction_output_video(video_path='IMG_0914.MOV', out_video_path='IMG_0914-output_zzz.MOV'):
    # 指定影片路徑以及輸出路徑的名稱
    ...
    # --------------------------- #selfchange_ed2&ed3&ed4_膝蓋平行&膝蓋傾斜&臀部下壓&背部挺直 分數平滑
    # Initialize knees score smoothing.
    knees_score_filter = Score_smoothing(window_size=10,
                                         alpha=0.2)
    knees_tilt_filter = Score_smoothing(window_size=10,
                                        alpha=0.2)
    pipe_down_filter = Score_smoothing(window_size=10,
                                       alpha=0.2)
    back_straight_filter = Score_smoothing(window_size=10,
                                           alpha=0.2)
    # ---------------------------
    ...
    with tqdm.tqdm(total=video_n_frames, position=0, leave=True) as pbar:
        # ========================= #selfchange_ed2&ed3&ed4_各種拿來應用的初始值設定 (2021/12/22)
        test2_list = []
        switch_list = []
        switch_knees_parallel_list = []  # ed2
        switch_knees_tilt_list = []  # ed3
        switch_pipe_down_list = []  # ed4
        switch_back_straight_list = []  # ed4
        switch = False
        knees_parallel_min_score = 0  # ed2
        knees_tilt_score = 0  # ed3
        pipe_down_score = 0  # ed4
        back_straight_score = 0  # ed4
        final_list = []
        # =========================
        while True:
            ...
            if pose_landmarks is not None:
                ...
                # ------------------------------- #selfchange_ed1&ed2&ed3&ed4_自定義函數實際套用的位置 (功能: 計算與正確姿勢的距離誤差)
                ...
                knees_tilt_analyze = pose_classifier.find_knees_tilt(pose_distance_error)
                pipe_down_analyze = pose_classifier.find_pipe_down(pose_distance_error)
                back_straight_analyze = pose_classifier.find_back_straight(pose_distance_error)

                knees_score_filtered = knees_score_filter(knees_analyze)
                knees_tilt_filtered = knees_tilt_filter(knees_tilt_analyze)
                pipe_down_filtered = pipe_down_filter(pipe_down_analyze)
                back_straight_filtered = back_straight_filter(back_straight_analyze)
                ...
                # ------------------------------- #selfchange_ed2&ed3&ed4_各種開關資訊獲取 (2021/12/22)
                switch = repetition_counter.get_switch()
                switch_list.append(switch)
                if switch == True:
                    switch_knees_parallel_list.append(knees_score_filtered)  # ed2
                    switch_knees_tilt_list.append(knees_tilt_filtered)  # ed3
                    switch_pipe_down_list.append(pipe_down_filtered)  # ed4
                    switch_back_straight_list.append(back_straight_filtered)  # ed4
                if len(switch_list) >= 2:
                    if switch_list[-2] == True:
                        if switch == False:
                            knees_parallel_min_score = np.min(switch_knees_parallel_list)
                            knees_tilt_min_score = np.min(switch_knees_tilt_list)
                            knees_tilt_max_score = np.max(switch_knees_tilt_list)
                            knees_tilt_score = knees_tilt_max_score * 0.5 + knees_tilt_min_score * 0.5
                            pipe_down_min_score = np.min(switch_pipe_down_list)
                            pipe_down_max_score = np.max(switch_pipe_down_list)
                            pipe_down_score = pipe_down_max_score * 0.5 + pipe_down_min_score * 0.5
                            back_straight_min_score = np.min(switch_back_straight_list)
                            back_straight_max_score = np.max(switch_back_straight_list)
                            back_straight_score = back_straight_max_score * 0.5 + back_straight_min_score * 0.5

                            switch_knees_parallel_list = []
                            switch_knees_tilt_list = []
                            switch_pipe_down_list = []
                            switch_back_straight_list = []

                            # ==================
                            final_list.append([repetition_counter.n_repeats,
                                               math.floor(knees_parallel_min_score),
                                               math.floor(knees_tilt_score),
                                               math.floor(pipe_down_score),
                                               math.floor(back_straight_score)])
                            print(f'目前深蹲累計次數 : {final_list[-1][0]} , 四個分數分別為 : {final_list[-1][1:]}')
                            # ==================
                # -------------------------------
            ...
            output_frame = pose_classification_visualizer(
                ...
                # ----------------------------  #selfchange_ed2&ed3&ed4_增加參數
                knees_score=math.floor(knees_parallel_min_score),
                knees_tilt=math.floor(knees_tilt_score),
                pipe_down=math.floor(pipe_down_score),
                back_straight=math.floor(back_straight_score))
            # ----------------------------
...
##12. 即時攝影鏡頭深蹲動作預測
def camera_realtime_predict(video_width=640, video_height=480, video_cap = cv2.VideoCapture(0)):
    class_name = 'down'
    ...
    while True:
        ...
        # =======================================================
        #     output_frame = draw_score(output_frame,repetitions_count,knees_parallel_min_score,knees_tilt_min_score)
        cv2.putText(output_frame, "Squat Count:", (int(video_width * 0.45), int(video_height * 0.05)),
                    cv2.FONT_HERSHEY_TRIPLEX, 1.2, (255, 0, 128), 2)
        cv2.putText(output_frame, str(repetitions_count), (int(video_width * 0.73), int(video_height * 0.2)),
                    cv2.FONT_HERSHEY_TRIPLEX, 3, (255, 0, 128), 2)
        cv2.putText(output_frame, "Knees Parallel Score:", (int(video_width * 0.4), int(video_height * 0.3)),
                    cv2.FONT_HERSHEY_TRIPLEX, 1, (255, 0, 0), 2)
        cv2.putText(output_frame, str(math.floor(knees_parallel_min_score)),
                    (int(video_width * 0.7), int(video_height * 0.4)), cv2.FONT_HERSHEY_TRIPLEX, 2, (255, 0, 0), 2)
        cv2.putText(output_frame, "Knees Tilt Score:", (int(video_width * 0.45), int(video_height * 0.5)),
                    cv2.FONT_HERSHEY_TRIPLEX, 1, (255, 0, 0), 2)
        cv2.putText(output_frame, str(math.floor(knees_tilt_score)), (int(video_width * 0.7), int(video_height * 0.6)),
                    cv2.FONT_HERSHEY_TRIPLEX, 2, (255, 0, 0), 2)
        cv2.putText(output_frame, "Pipe Down Score:", (int(video_width * 0.45), int(video_height * 0.7)),
                    cv2.FONT_HERSHEY_TRIPLEX, 1, (255, 0, 0), 2)
        cv2.putText(output_frame, str(math.floor(pipe_down_score)), (int(video_width * 0.7), int(video_height * 0.8)),
                    cv2.FONT_HERSHEY_TRIPLEX, 2, (255, 0, 0), 2)
        cv2.putText(output_frame, "Back Straight Score:", (int(video_width * 0.4), int(video_height * 0.9)),
                    cv2.FONT_HERSHEY_TRIPLEX, 1, (255, 0, 0), 2)
        cv2.putText(output_frame, str(math.floor(back_straight_score)),
                    (int(video_width * 0.7), int(video_height * 1.0)), cv2.FONT_HERSHEY_TRIPLEX, 2, (255, 0, 0), 2)
        # =====================================================

        frame_idx += 1
        output_frame = cv2.cvtColor(output_frame, cv2.COLOR_RGB2BGR)
        cv2.imshow("testvideo", output_frame)
        if cv2.waitKey(1) & 0xff == ord("q"):
            break

    # Release MediaPipe resources.
    pose_tracker.close()
    print(f'總共完成深蹲次數: {repetition_counter.n_repeats}')
